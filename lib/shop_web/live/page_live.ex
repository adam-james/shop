defmodule ShopWeb.PageLive do
  ...
  def mount(params, %{"cart" => cart}, socket) do
    ...
    {:ok, assign(socket, page: page, cart: cart, products_added: 1)}
  end
  ...
  def handle_event("add-to-cart", %{"id" => id} = _value, socket) do
    ...
    socket =
      socket |> put_flash(:info, "Product added to cart (x #{socket.assigns.products_added}).")

    {:noreply, assign(socket, cart: cart, products_added: socket.assigns.products_added + 1)}
  end
